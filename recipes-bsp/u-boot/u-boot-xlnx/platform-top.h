...
#define CONFIG_EXTRA_ENV_SETTINGS               \
    ...
    "machine_name=carbon808\0" \
    "uenvcmd=run bootkernel\0" \
    "devicetree_image=carbon808.dtb\0" \
    "kernel_image=image\0" \
    "bootargs=earlycon clk_ignore_unused root=/dev/mmcblk0p2 rw rootwait\0" \
    "append_fpgadev=env set bootargs ${bootargs} fpgadev=${fpgadev}\0" \
    "loadkernel=fatload mmc 0 ${kernel_load_address} ${kernel_image}\0" \
    "loaddtb=fatload mmc 0 ${devicetree_load_address} ${devicetree_image}\0" \
    "loadbitfile=fatload mmc 0 ${bitfile_load_address} download.${fpgadev}.bin && fpga load 0 ${bitfile_load_address} $filesize\0" \
